<script lang="ts">
	...
	import type { GameClass } from '$lib/types/encyclopedia';
	import { onMount } from 'svelte';

	let classes = $state<GameClass[]>([]);
...
<div class="min-h-screen grid-bg p-8">
		...
		<div class="max-w-7xl mx-auto px-4 py-4">
			<div class="flex items-center justify-between">
				...
				<div class="flex gap-4">
					...
					<a href="/classes" class="text-white">Classes</a>
	...
	<main class="max-w-7xl mx-auto px-4">
		<h2 class="font-display text-3xl font-bold text-slate-100 mb-6">Classes</h2>

		{#if loading}
			<p class="text-slate-400">Loading...</p>
		{:else}
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{#each classes as gameClass (gameClass.id)}
					<div class="class-card glass rounded-lg p-4">
						<h3 class="class-name">{gameClass.name}</h3>
						<p class="class-description">{gameClass.description}</p>
						<div class="synergies mt-4">
							{#each gameClass.synergyLevels as level}
